...
type Cluster struct {
	...
	mongos   *util.Mongo
	shards   map[string]*util.Mongo
	mongopbm *util.MongoPBM
	confsrv  string
...
func New(cfg ClusterConf) *Cluster {
	...
	c := &Cluster{
		...
		shards:   make(map[string]*util.Mongo),
	...
	pbmObj, err := util.NewCtl(c.ctx, cfg.DockerSocket)
	if err != nil {
	...
	c.docker, err = util.NewDocker(c.ctx, cfg.DockerSocket)
	if err != nil {
...
func mgoConn(ctx context.Context, uri string) *util.Mongo {
	m, err := util.NewMongo(ctx, uri)
	if err != nil {
		log.Fatalln("connect to mongo:", err)
	}
...
func pbmConn(ctx context.Context, uri string) *util.MongoPBM {
	m, err := util.NewMongoPBM(ctx, uri)
	if err != nil {
		log.Fatalln("connect to mongo:", err)
	}
	log.Println("connected to", uri)
