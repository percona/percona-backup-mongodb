...
import (
	...
	"github.com/percona/percona-backup-mongodb/e2e-tests/runner/util"
)

type scounter struct {
	data   <-chan *[]util.Counter
...
func (c *Cluster) bcheckClear(name string, shard *util.Mongo) {
...
func (c *Cluster) bcheckWrite(name string, shard *util.Mongo, t time.Duration) (<-chan *[]util.Counter, context.CancelFunc) {
	var data []util.Counter
	ctx, cancel := context.WithCancel(c.ctx)
	dt := make(chan *[]util.Counter)
	go func() {
...
func (c *Cluster) bcheckCheck(name string, shard *util.Mongo, data *[]util.Counter, bcpLastWrite primitive.Timestamp, inRange func(ts, limit primitive.Timestamp) bool) {
