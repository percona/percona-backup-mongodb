...
func newpcounter(shard string, cn *util.Mongo) *pcounter {
	return &pcounter{
		shard: shard,
		cn:    cn,
	}
...
func (pc *pcounter) current() *util.Counter {
	pc.mx.Lock()
	defer pc.mx.Unlock()
	return pc.curr
}

func (c *Cluster) pitrcCheck(name string, shard *util.Mongo, data *[]util.Counter, bcpLastWrite primitive.Timestamp) {
