...
type CleanupInfo struct {
	Backups []backup.BackupMeta `json:"backups"`
...
func MakeCleanupInfo(ctx context.Context, conn connect.Client, ts primitive.Timestamp) (CleanupInfo, error) {
	...
	ok, err := canDeleteBaseSnapshot(ctx, conn, ts)
	if err != nil {
...
func listBackupsBefore(ctx context.Context, conn connect.Client, ts primitive.Timestamp) ([]backup.BackupMeta, error) {
	...
	rv := []backup.BackupMeta{}
	err = cur.All(ctx, &rv)
	return rv, errors.Wrap(err, "cursor: all")
}

func canDeleteBaseSnapshot(ctx context.Context, conn connect.Client, lw primitive.Timestamp) (bool, error) {
	f := bson.D{
	...
	err := conn.BcpCollection().FindOne(ctx, f, o).Err()
	if err == nil {
...
func findLastBaseSnapshotIndex(bcps []backup.BackupMeta) int {
...
func isBaseSnapshot(bcp *backup.BackupMeta) bool {
