...
// This test is almost the same as TestBasicApplyLog with the only difference that in before trying
// to apply the oplog, we are going to read the dump file using bson.Reader, we are going to get
// the timestamt of the document in the middle of the dump (document 25) and we wer going to apply
// the oplog until THAT timestamp. As the result, we should have only 25 documents after the apply.
func TestApplyLogUntil(t *testing.T) {
	tmpfile, err := ioutil.TempFile("", "example.bson.")
	if err != nil {
		log.Fatal(err)
	}
	...
	middleDoc, err := getOplogDoc(tmpfile.Name(), docCount/2)
	if err != nil {
		t.Fatalf("cannot open oplog dump file %s: %s", tmpfile.Name(), err)
	}
	// Open the oplog file we just wrote and use it as a reader for the oplog apply
	reader, err := bsonfile.OpenFile(tmpfile.Name())
	if err != nil {
	...
	// Replay the oplog
	oa, err := NewOplogApplyUntil(session, reader, middleDoc["ts"].(bson.MongoTimestamp))
	if err != nil {
		t.Errorf("Cannot instantiate the oplog applier: %s", err)
	}
	...
	if i != docCount/2 {
		t.Errorf("Invalid document count after repalying the oplog. Want %d, got %d", docCount/2, i)
	}
}

func getOplogDoc(filename string, docNumber int) (bson.M, error) {
	...
	for {
		...
		// We are only interested on the inserts to the test.test_collection
		if doc["op"].(string) != "i" || doc["ns"].(string) != "test.test_collection" {
			continue
		}
		i++
		if i == docNumber {
	...
	return nil, fmt.Errorf("cannot find ducumet at position %d", docNumber)
}
