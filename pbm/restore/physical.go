...
func NewPhysical(
	...
	rcf, err := topo.GetReplSetConfig(ctx, node.Session())
	if err != nil {
	...
	if inf.IsSharded() {
		...
		if inf.ReplsetRole() != topo.RoleConfigSrv {
			csvr, err = topo.ConfSvrConn(ctx, node.Session())
			if err != nil {
...
func (r *PhysRestore) flush(ctx context.Context) error {
	...
	rsStat, err := topo.GetReplsetStatus(ctx, r.node.Session())
	if err != nil {
	...
	for {
		inf, err := topo.GetNodeInfoExt(ctx, r.node.Session())
		if err != nil {
...
func (r *PhysRestore) Snapshot(
	...
	meta := &types.RestoreMeta{
		...
		Replsets: []types.RestoreReplset{{Name: r.nodeInfo.Me}},
	}
	...
	if !pitr.IsZero() {
		opChunks, err = chunks(ctx, r.cn, r.stg, r.restoreTS, pitr, r.rsConf.ID, r.rsMap)
		if err != nil {
			return err
		}
	}
	...
	var stats types.RestoreShardStat
...
func (r *PhysRestore) cleanupDatadir(bcpFiles []types.File) error {
...
func (r *PhysRestore) dumpMeta(meta *types.RestoreMeta, s defs.Status, msg string) error {
	...
	condsm, err := resync.GetPhysRestoreMeta(meta.Name, r.stg, r.log)
	if err == nil {
	...
	if err != nil || s == defs.StatusError {
		ts := time.Now().Unix()
		meta.Status = s
		meta.Conditions = append(meta.Conditions, &types.Condition{Timestamp: ts, Status: s})
...
func (r *PhysRestore) getShardMapping(bcp *types.BackupMeta) map[string]string {
...
func (r *PhysRestore) setcommittedTxn(_ context.Context, txn []types.RestoreTxn) error {
	if txn == nil {
...
func (r *PhysRestore) init(ctx context.Context, name string, opid types.OPID, l *log.Event) error {
	cfg, err := config.GetConfig(ctx, r.cn.Conn)
	if err != nil {
		return errors.Wrap(err, "get pbm config")
	}

	r.stg, err = util.StorageFromConfig(cfg, l)
	if err != nil {
...
func (r *PhysRestore) setBcpFiles(ctx context.Context) error {
	...
	for {
		...
		var err error
		bcp, err = query.GetBackupMeta(ctx, r.cn.Conn, bcp.SrcBackup)
		if err != nil {
...
func getRS(bcp *types.BackupMeta, rs string) *types.BackupReplset {
...
func (r *PhysRestore) prepareBackup(ctx context.Context, backupName string) error {
	var err error
	r.bcp, err = query.GetBackupMeta(ctx, r.cn.Conn, backupName)
	...
	err = query.SetRestoreBackup(ctx, r.cn.Conn, r.name, r.bcp.Name, nil)
	if err != nil {
...
func (r *PhysRestore) MarkFailed(meta *types.RestoreMeta, e error, markCluster bool) {
	...
	if errors.As(e, &nerr) {
		e = nerr
		meta.Replsets = []types.RestoreReplset{{
			Name:   nerr.node,
