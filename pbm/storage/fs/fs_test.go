...
func TestGetRestoreBuffSize(t *testing.T) {
	const (
		minBuffSize = 32 * 1024
		maxBuffSize = 10 * 1024 * 1024
	)
	...
	tests := []struct {
		...
		{
			name: "negative value returns zero",
			cfg:  &Config{RestoreBuffSize: -1},
			want: 0,
		},
		{
			name: "below lower bound is normalized to min",
			cfg:  &Config{RestoreBuffSize: 1024},
			want: minBuffSize,
		},
		{
			name: "one byte below lower bound is normalized to min",
			cfg:  &Config{RestoreBuffSize: minBuffSize - 1},
			want: minBuffSize,
		},
		{
			name: "at lower bound returns configured value",
			cfg:  &Config{RestoreBuffSize: minBuffSize},
			want: minBuffSize,
		},
		{
			name: "within range returns configured value",
			cfg:  &Config{RestoreBuffSize: 1 * 1024 * 1024},
			want: 1 * 1024 * 1024,
		},
		{
			name: "at upper bound returns configured value",
			cfg:  &Config{RestoreBuffSize: maxBuffSize},
			want: maxBuffSize,
		},
		{
			name: "one byte above upper bound is normalized to max",
			cfg:  &Config{RestoreBuffSize: maxBuffSize + 1},
			want: maxBuffSize,
		},
		{
			name: "above upper bound is normalized to max",
			cfg:  &Config{RestoreBuffSize: 20 * 1024 * 1024},
			want: maxBuffSize,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := tt.cfg.GetRestoreBuffSize()
			if got != tt.want {
				t.Errorf("GetRestoreBuffSize: got=%d, want=%d", got, tt.want)
			}
		})
	}
}
